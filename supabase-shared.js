(function () {
  ...
  async function loadDirectory() {
    ...
    const render = () => {
      ...
      directory.querySelectorAll("[data-live-member]").forEach((button) => button.addEventListener("click", () => {
        ...
        document.getElementById("member-dialog-content").innerHTML = `<p class="eyebrow">${escapeHtml(member.rank)}</p><h2 id="member-dialog-name">${escapeHtml(member.display_name)}</h2><p class="profile-subtitle">${main ? `${escapeHtml(main.name)} Â· ${escapeHtml(main.specialization)} ${escapeHtml(main.class_name)} Â· ${escapeHtml(main.realm)}` : "No main character selected"}</p><p>${escapeHtml(member.bio || "No member biography yet.")}</p><dl class="event-facts"><div><dt>Discord</dt><dd>${escapeHtml(member.discord_name || "Not shared")}</dd></div><div><dt>Characters</dt><dd>${member.characters.map((item) => `${escapeHtml(item.name)} Â· ${escapeHtml(item.primary_role)}${item.is_main ? " Â· Main" : ""}`).join("<br>") || "None"}</dd></div></dl>`;
  ...
  async function submitRecruitment() {
    ...
    form.addEventListener("submit", async (event) => {
      event.preventDefault(); const message = document.getElementById("application-message"); message.textContent = "Submittingâ€¦";
  ...
  async function loadCalendar() {
    ...
    const charSelect=document.getElementById("signup-character"); charSelect.innerHTML=user ? `<option value="">Choose a character</option>${state.characters.map((item)=>`<option value="${item.id}" data-role="${item.primary_role}">${escapeHtml(item.name)} Â· ${escapeHtml(item.primary_role)}</option>`).join("")}` : `<option value="">Sign in to choose a character</option>`;
    ...
    async function openEvent(item){
      state.current=item;const [label,color]=categories[item.category];document.getElementById("event-dialog-category").textContent=label;document.getElementById("event-dialog-category").style.setProperty("--event-color",color);document.getElementById("event-dialog-title").textContent=item.title;document.getElementById("event-dialog-time").textContent=`${item.date.toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"})} Â· ${item.date.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"})} Â· ${Math.round(item.duration_minutes/60*10)/10} hours`;document.getElementById("event-dialog-description").textContent=item.description;document.getElementById("event-dialog-location").textContent=item.location;document.getElementById("event-dialog-lead").textContent=item.organizer;document.getElementById("event-dialog-requirements").textContent=item.requirements;document.getElementById("event-dialog").showModal();
