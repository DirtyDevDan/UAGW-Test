(async function () {
  ...
  async function loadApplications() {
    ...
    $("shared-application-list").innerHTML = applications.map((item) => `<article class="application-card panel"><div class="application-title"><div><span>${esc(item.status)}</span><h3>${esc(item.character_name)}</h3><p>${esc(item.class_name)} · ${esc(item.primary_role)} · ilvl ${item.item_level || "—"}</p></div><time>${new Date(item.created_at).toLocaleDateString()}</time></div><dl class="event-facts"><div><dt>Email</dt><dd>${esc(item.email)}</dd></div><div><dt>Discord</dt><dd>${esc(item.discord_name)}</dd></div><div><dt>Goals</dt><dd>${esc(item.goals)}</dd></div><div><dt>Experience</dt><dd>${esc(item.experience)}</dd></div></dl><label>Leadership decision<select data-review-app="${item.id}"><option value="New" ${item.status === "New" ? "selected" : ""}>New</option><option value="Reviewing" ${item.status === "Reviewing" ? "selected" : ""}>Reviewing</option><option value="Interview" ${item.status === "Interview" ? "selected" : ""}>Interview</option><option value="Accepted" ${item.status === "Accepted" ? "selected" : ""}>Accept &amp; unlock member access</option><option value="Declined" ${item.status === "Declined" ? "selected" : ""}>Decline &amp; keep access locked</option></select></label></article>`).join("") || `<p class="empty-state">No applications yet.</p>`;
    document.querySelectorAll("[data-review-app]").forEach((select) => select.addEventListener("change", async () => {
      ...
      if (["Accepted", "Declined"].includes(select.value) && !window.confirm(`${select.value === "Accepted" ? "Accept" : "Decline"} ${application?.character_name || "this application"}? ${select.value === "Accepted" ? "This immediately unlocks all member-only pages." : "Member access will remain locked."}`)) { select.value = application.status; return; }
